...
object ScanamoFree {
  ...
  def transactPutAllTable[T](
    tableName: String
  )(items: List[T])(implicit f: DynamoFormat[T]): ScanamoOps[Either[ConditionalCheckFailedException,TransactWriteItemsResponse]] =
    transactPutAll(items.map(tableName -> _))

  def transactPutAll[T](
    tableAndItems: List[(String, T)]
  )(implicit f: DynamoFormat[T]): ScanamoOps[Either[ConditionalCheckFailedException,TransactWriteItemsResponse]] = {
  ...
  def transactUpdateAllTable(
    tableName: String
  )(items: List[(UniqueKey[_], UpdateExpression)]): ScanamoOps[Either[ConditionalCheckFailedException,TransactWriteItemsResponse]] =
  ...
  def transactDeleteAllTable(
    tableName: String
  )(items: List[UniqueKey[_]]): ScanamoOps[Either[ConditionalCheckFailedException,TransactWriteItemsResponse]] =
